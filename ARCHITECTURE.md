...
"이 정점은 어느 뼈를 따라가야 하나"는 두 뼈가 공간에서 어떻게 놓였는지에 따라 **다른 질문**이다.

| 규칙 | 쓰는 곳 | 묻는 것 |
|---|---|---|
| **ALONG** | 팔꿈치·무릎·손목·발목 | "관절에서 팔다리 축을 따라 얼마나 지났나" |
| **NEAR** | 어깨·엉덩관절 (몸통→팔다리) | "팔다리에 실제로 얼마나 가까운가" |

같은 뼈의 연장선상에 있는 두 뼈(위팔↔아래팔)는 **거리로 구분할 수 없다** — 위팔근 힘살은
아래팔뼈에서 5cm밖에 안 떨어져 있지만 위팔뼈 소속이다. 그래서 ALONG.

반대로 몸통 뼈와 팔다리 뼈는 방향 자체가 다르다. 몸통에 붙은 근육은 **몸통을 따라** 뻗지
팔다리를 따라 뻗지 않으므로, ALONG을 쓰면 광배근의 허리 끝까지 전부 "어깨보다 아래"로
읽혀 위팔뼈에 묶인다(실측: 정점 100%). 여기선 근접도가 답이다 — 힘줄만 위팔뼈에 닿고
힘살은 절대 닿지 않는다.

몸통→몸통 span(복근, 척주세움근)은 몸통을 따라 일직선이므로 다시 ALONG이다.
...
- **컨텍스트 근육 312개 중 239개는 강조 불가**(id 0) — 실루엣용. 동작 데이터에 있는
  73개만 강조·이름 표시 대상
- **어깨뼈가 안 움직임** — 어깨 벌림 90° 이상에서 해부학적으로는 어깨뼈가 위쪽
  회전해야 하지만 리그에 그 자유도가 없다. `movements.ts`가 앞톱니근·등세모근을
  협력근으로 정확히 나열하고는 있으나, 그 움직임 자체는 표현되지 않는다
