...
네이티브 `dblclick`은 터치에서 신뢰 불가 (iOS는 더블탭-줌 때문에 역사적으로 300ms 지연까지 있었음).
`onPointerDown`에서 수동 판정:

```
같은 오브젝트 && 이전 탭으로부터 300ms 이내 && 이동거리 < 10px  → 더블탭
```
...
└─ src/
   ...
   │  ├─ CameraRig.tsx        OrbitControls: 좌우 180° / 상하 120°, 핀치줌, 팬 비활성
...
- [x] **P1** 스캐폴드 + 흰 배경 캔버스 + 카메라 제한(180°/120°) + 핀치줌 + placeholder 마네킹
